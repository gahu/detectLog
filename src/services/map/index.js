...
const DAUM_KAKAO_API_JS_KEY = '938089d746f95b6de32b253b7dae17be'
...
class Map {
    ...
    async mount (elementId) {
        ...
        if (Map.cachedMaps[elementId]) {
            ...
            this.map = new Map.daum.maps.Map(
                ...
                {
                    center: new Map.daum.maps.LatLng(37.22292328, 127.19213867),
                    level: 13,
                },
            )
    ...
    addMarkers (markerSpecs = []) {
        const markerSpecWithoutClusterKey = []
        const markerSpecsByClusterKey = markerSpecs.reduce((result, spec) => {
            if (!spec.clusterKey) {
                markerSpecWithoutClusterKey.push(spec)
                return result
            }
        ...
        markerSpecWithoutClusterKey.forEach(({ lat, lng, title = null, onClick = null }) => {
